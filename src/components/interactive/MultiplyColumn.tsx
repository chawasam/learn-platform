...
export default function MultiplyColumn({
  ...
  // cols: place-value indices rendered left→right (highest place first)
  ...
  return (
    <div className="flex flex-col items-center gap-4 select-none">
      <div className="rounded-2xl bg-slate-50 px-5 py-4 border border-gray-100">
        {/* CSS Grid: all cells in the same logical row share the same height.
            This guarantees the result row is a single horizontal plane regardless
            of which columns have empty A or B cells (e.g. the final-carry column). */}
        <div
          className="font-bold text-3xl"
          style={{
            display: 'grid',
            gridTemplateColumns: `repeat(${totalCols}, 2.5rem)`,
            columnGap: '0.375rem',
            fontVariantNumeric: 'tabular-nums',
          }}
        >
          {/* Row 1: carry markers above each column */}
          {cols.map(c => {
            const carryVal = c > 0 && c < len ? steps[c - 1].carryOut : 0
            const showCarry = carryVal > 0 && shown > c - 1
            return (
              <div key={`carry-${c}`} className="h-6 flex items-end justify-center">
                <AnimatePresence>
                  {showCarry && (
                    <motion.span
                      key={`cv-${c}`}
          ...
          {/* Row 2: top number A digits */}
          {cols.map(c => (
            <div key={`a-${c}`} className="h-9 flex items-center justify-center" style={{ color: '#1E3A5F' }}>
              {c < len ? A[c] : ''}
            </div>
          ))}

          {/* Row 3: multiplier b in ones column, × sign to its left */}
          {cols.map(c => (
            <div key={`b-${c}`} className="h-9 flex items-center justify-center relative" style={{ color: '#1E3A5F' }}>
              {c === 0 && (
          ...
          <div className="col-span-full h-0.5 bg-gray-400 my-1" />

          {/* Row 4: result digits — all rendered in the same grid row = same baseline */}
          {cols.map(c => (
            <div key={`r-${c}`} className="h-9 flex items-center justify-center">
