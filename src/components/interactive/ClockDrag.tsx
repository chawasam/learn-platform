...
export default function ClockDrag({
  ...
  const markers = Array.from({ length: 60 }, (_, i) => {
    ...
    return {
      x1: cx + inner * Math.cos((a * Math.PI) / 180),
      y1: cy + inner * Math.sin((a * Math.PI) / 180),
      x2: cx + (r - 2) * Math.cos((a * Math.PI) / 180),
      y2: cy + (r - 2) * Math.sin((a * Math.PI) / 180),
      major,
      label: major ? i / 5 || 12 : null,
      labelX: cx + (r - 24) * Math.cos((a * Math.PI) / 180),
      labelY: cy + (r - 24) * Math.sin((a * Math.PI) / 180),
    }
  })
